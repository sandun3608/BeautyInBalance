...
const regex = /<div class="mobile-drawer" id="mobile-drawer">[\s\S]*?<div class="drawer-footer">[\s\S]*?<\/div>\s*<\/div>/;

for (const file of files) {
  let content = fs.readFileSync(path.join(dir, file), 'utf8');
  
  if (regex.test(content)) {
    content = content.replace(regex, newDrawerHTML);
    fs.writeFileSync(path.join(dir, file), content, 'utf8');
    console.log('Updated', file);
  } else {
    console.log('Skipped', file, ' (mobile-drawer not found or format mismatch)');
  }
}
